...
import { r as renderComponent } from "./worker-entry_BMlXVnqT.mjs";
import { kvPut } from "./kv_Bpi6S22S.mjs";
import { env } from "cloudflare:workers";
import { $ as $$App } from "./App_iuB77n_u.mjs";
...
const $$Dashboard = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { getSession } = await import("./auth_Bpgz-Z3X.mjs");
  ...
  const { pollHost, cachedEntitlement, APP_VERSION, nowSec, hasApiKey } = await import("./client_BlXSQX-O.mjs");
  const AI_PROVIDERS = [
  ...
  const { cmpVersion } = await import("./update_BHRUc_yu.mjs");
  await import("./index_CH4ctGP-.mjs");
  const { appCatalog, installedAppIds } = await import("./apps_CR0L9h8o.mjs");
  const { getStorageUsage, fmtBytes } = await import("./storage-usage_CV8Ic_-f.mjs");
  ...
  const { reviewQueue } = await import("./users_CyuckN8p.mjs");
  const { backupAlert } = await import("./backup_iNKGYUoP.mjs");
  ...
  const { installedAppLaunchers } = await import("./external-apps_alh5RdSP.mjs").then((n) => n.C);
  ...
  const { storageMode } = await import("./storage_D0AXCVqV.mjs");
