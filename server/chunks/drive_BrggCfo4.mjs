...
const $$Drive = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { cachedEntitlement } = await import("./client_BlXSQX-O.mjs");
  const hasPlus = atLeast(await cachedEntitlement(env), "plus");
  const { driveConnected, driveAvailable, driveStatus, listDriveFiles, getDriveSave } = await import("./drive_CvXOMGpW.mjs");
