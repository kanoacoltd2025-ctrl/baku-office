...
const $$Apps = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { atLeast, planLabel } = await import("./ctx_CoGRP8-y.mjs").then((n) => n.L);
  const entitlement = await (await import("./client_BlXSQX-O.mjs")).cachedEntitlement(env).catch(() => "free");
  const okPlan = (minPlan) => !minPlan || atLeast(entitlement, minPlan);
  await import("./index_CH4ctGP-.mjs");
  const { appCatalog, installedAppIds, MANDATORY_APPS } = await import("./apps_CR0L9h8o.mjs");
  const { registeredParts } = await import("./parts_CYwgYHWx.mjs").then((n) => n.f);
  const { listExternalApps, listDrafts, installedAppLaunchers, isRegisteredPart, reconcileInstalledParts } = await import("./external-apps_alh5RdSP.mjs").then((n) => n.C);
  const { storeCatalog, myApps } = await import("./store_Cva0cZFS.mjs");
