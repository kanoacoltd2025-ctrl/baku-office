...
import { getToken, hostFetch, getVerifyJwk } from "./client_BlXSQX-O.mjs";
...
import { p as preflight, i as isRunnableDefinition } from "./preflight_YrUnL5aD.mjs";
import { sanitizeRenderHtml, validateDefinition, isBlockedHost } from "./appdef_C41c_UbP.mjs";
import { a as isCuratedHost } from "./cdn-allowlist_rKupC5M_.mjs";
import { r as registeredParts } from "./parts_CYwgYHWx.mjs";
import { installApp } from "./apps_CR0L9h8o.mjs";
const MAX_TAGS = 8;
...
async function installLocalApp(ctx, draftId, by) {
  ...
  if (d.definition) {
    try {
      const jsErr = (await import("./appdef_C41c_UbP.mjs")).checkRenderScripts(JSON.parse(d.definition).render?.html ?? "");
    ...
    try {
      ...
      if (parsed.render?.isolation === "relaxed") {
        ...
        if (okApproved) {
          ...
          const { createApproval } = await import("./approvals_BefKqvVJ.mjs");
