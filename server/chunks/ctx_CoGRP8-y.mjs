...
import { getFile, saveFile, setRetentionDays, setMaxUploadMb, storageMode, fileBelongsTo } from "./storage_D0AXCVqV.mjs";
import { getApiKey, getToken, hostFetch, cachedEntitlement, entitlementForGate, masterKeyCtx } from "./client_BlXSQX-O.mjs";
import { n as nowSec } from "./accounting_D4tRmfws.mjs";
import { addNotification, notifyHook } from "./notifications_31cgBE37.mjs";
import { createInvite, approveUser, setRole, activeAdminCount, rejectUser, deleteUser, listUsers, verifyPassword } from "./users_CyuckN8p.mjs";
import { detectProfile } from "./profiles_D3vLhBYo.mjs";
import { permissionCatalogText, ALLOWED_PERMISSIONS, installedAppIds, appCatalog, installApp, uninstallApp, makeAppsApi } from "./apps_CR0L9h8o.mjs";
import "./index_CH4ctGP-.mjs";
...
import { googleFetch } from "./google_BprIwX8C.mjs";
...
import { APP_SCHEMA, RENDER_HTML_MAX, validateDefinition, checkRenderScripts, checkRenderHandlers, checkRenderDataKeys, checkRenderScreens, opCatalogText, googleOpCatalogText } from "./appdef_C41c_UbP.mjs";
import { turnMedia, runToolLoop, HOPS_EXCEEDED, classifyModelError } from "./ai_CSVvSxX0.mjs";
import { a2aHost, callPartner, groupRelayCall, callPublic, sendInquiry } from "./a2a_D7TcTW5m.mjs";
import { getOrgProfile, setOrgProfile, listPublicActions, listActions, createAction, deleteAction, updateAction } from "./a2a-actions_BKoWg0s5.mjs";
import { brandName, getTheme } from "./theme_DFty9gzU.mjs";
import { setAutonomy, getAutonomyConfig, autonomyReady, AUTONOMY_TOOLS, AUTONOMY_POLICY, runAutonomyTool } from "./autonomy_HywyH5YN.mjs";
import { g as runApp, p as platformInvariantSuspected, e as escalatePlatformInvariant, a as runInstalledApp, r as runDraftApp, f as appRunNeedsApproval, b as authorizeAppRun, h as googleToolPreflight } from "./app-runtime_CqA25oqJ.mjs";
import { logDiag } from "./diag_NtHEh0X8.mjs";
import { i as isRunnableDefinition } from "./preflight_YrUnL5aD.mjs";
...
import { i as isDisplayOnly, u as unitsOf, a as checkTransitions, s as sampleInputs } from "./precheck_DEHfd6jR.mjs";
...
import { listSkills, runSkill, generateSkill, searchSkills } from "./skills_DcA02trz.mjs";
import { storeCatalog } from "./store_Cva0cZFS.mjs";
import { listCapabilities, capabilitySummary, videoStatusText, invokeCapability } from "./capabilities_USXyoOsU.mjs";
import { setApprovalMode, getApprovalMode, createApproval, A2A_OUTWARD, previewFor } from "./approvals_BefKqvVJ.mjs";
import { saveLayoutDraft, listSites } from "./sites_MPH9v24n.mjs";
...
import { p as pushToUser } from "./push_CAUV5NTF.mjs";
...
${lines}`;
    ...
    if (action === "build") {
      ...
      const base = await import("./sites_MPH9v24n.mjs").then((m) => m.getSite(env, slug)).then((s) => {
        try {
...
・どうする：管理者がホーム画面の『スキル管理』で内容を確認して有効化すると、run_skill で使えるようになります。` : g.error ?? "スキル生成に失敗しました。";
    ...
    case "post_social": {
      ...
      const { socialPost } = await import("./social_DKmNq4a3.mjs");
    ...
    case "search_social": {
      const { socialSearch } = await import("./social_DKmNq4a3.mjs");
   ...
   ${i.url ?? ""}`).join("\n");
    ...
    case "read_social": {
      const { socialRead } = await import("./social_DKmNq4a3.mjs");
...
async function runAgent(ctx2, owner, text, image, baseUrl = "", role = "member", opts = {}) {
  ...
  const { disabledBuiltins } = await import("./client_BlXSQX-O.mjs");
  ...
  const snsOn = Object.values(await (await import("./social_DKmNq4a3.mjs")).socialStatus(env)).some(Boolean);
  ...
  const r2On = (await import("./storage_D0AXCVqV.mjs")).storageMode(env) === "r2";
