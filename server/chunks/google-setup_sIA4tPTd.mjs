...
const $$GoogleSetup = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const { googleConfigured, googleStatus, SCOPE_GROUPS } = await import("./google_BprIwX8C.mjs");
  const configured = isAdmin ? await googleConfigured(env) : false;
  const { getServiceAccountInfo } = await import("./google-sa_CKKtIJwX.mjs");
  const saInfo = isAdmin ? await getServiceAccountInfo(env).catch(() => null) : null;
  const { driveConfigured } = await import("./drive_CvXOMGpW.mjs");
  ...
  const { buildCtx } = await import("./ctx_CoGRP8-y.mjs").then((n) => n.X);
  ...
  const { randomId } = await import("./ctx_CoGRP8-y.mjs").then((n) => n.L);
