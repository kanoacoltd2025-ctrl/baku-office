...
import { updatePageMeta, setSitePassword, clearLayout, restoreLayoutVersion, listLayoutVersions, publishLayout, saveLayoutDraft, deleteSite, upsertSite } from "./sites_MPH9v24n.mjs";
import { v as validateLayout } from "./site-layout_IGnF6pBC.mjs";
import { audit } from "./storage_D0AXCVqV.mjs";
...
const POST = async ({ request, locals }) => {
  ...
  switch (b._action) {
    ...
    case "ai_layout": {
      ...
      if (b.ref?.dataB64) {
        const { prepareDevAttachment } = await import("./chat-flow_8TEyuDlF.mjs");
      ...
      const { startSiteBuild, processSiteBuild } = await import("./ctx_CoGRP8-y.mjs").then((n) => n.V);
    ...
    case "ai_status": {
      ...
      const { siteBuildStatus } = await import("./ctx_CoGRP8-y.mjs").then((n) => n.V);
      const st = await siteBuildStatus(locals.ctx, slug);
      let layout = null;
      if (st && st.status === "done") {
        const { getSite } = await import("./sites_MPH9v24n.mjs");
