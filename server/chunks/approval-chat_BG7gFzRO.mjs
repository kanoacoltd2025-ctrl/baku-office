...
import { getApproval, decideApproval } from "./approvals_BefKqvVJ.mjs";
import { currentAuthState, revokedSince, canDevelopApps } from "./auth_Bpgz-Z3X.mjs";
import { t as transcribeAudio, b as runApprovedTool, p as parseApprovalAction } from "./ctx_CoGRP8-y.mjs";
import { b as authorizeAppRun, c as authorizeDraftRun, v as verifyApprovalIntegrity } from "./app-runtime_CqA25oqJ.mjs";
import "./stripe_r-RFTlbb.mjs";
import { a as atLeast } from "./types_BVJxqWI9.mjs";
import { cachedEntitlement } from "./client_BlXSQX-O.mjs";
import { joinWithInvite } from "./users_CyuckN8p.mjs";
import { inboundFileLimitBytes, buildFallbackName, saveFile } from "./storage_D0AXCVqV.mjs";
...
async function joinViaInvite(env, connector, externalId, code, name) {
  ...
  if (inv?.target_user_id) {
    ...
    const { linkIdentity } = await import("./users_CyuckN8p.mjs");
...
${t}` : t;
  ...
  const uq = await import("./upload-queue_lUAA-_vr.mjs");
  const bins = [];
  const failedNames = [];
...
⚠️ ${failedNames.length}件は大きすぎるなどの理由で受け取れませんでした（1件あたり${limitMb}MBまで）。圧縮・分割してお送りいただくか、担当者にご相談ください。` : "";
  try {
    const { dispatchAppEvent } = await import("./app-events_BiQ3C9lx.mjs");
