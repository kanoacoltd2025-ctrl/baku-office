...
const POST = async ({ request, locals }) => {
  ...
  if (inboundFiles.length) {
    const { prepareAttachment } = await import("./chat-flow_8TEyuDlF.mjs");
  ...
  const { tryPreAgentRouting, tryProHopsContinuation, finalizeAssistantReply } = await import("./chat-flow_8TEyuDlF.mjs");
